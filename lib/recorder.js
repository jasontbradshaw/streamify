...
var Recorder = function (config) {
  ...
  // quit if we're not recording when a disconnect happens
...
Recorder.prototype._generateStreamId = function () {
  return this.streamPrefix + (new Date()).toISOString();
};

// update the parent as to the status of this recorder. should happen every time
// a significant event transpires.
Recorder.prototype._emitStatus = function () {
  this.socket.emit('status', this.recordingStreamId);
};

// if we're not recording when a disconnect happens, shut things down
Recorder.prototype._handleDisconnect = function () {
  if (!this.isRecording()) {
    this.exit();
  }
};
...
Recorder.prototype.record = function () {
  ...
  this.recordingStreamId = this._generateStreamId();

  // create the specified stream folder
  fs.mkdirSync(this.recordingStreamId);
