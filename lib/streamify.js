  ...
  if (segmentIndex === 0) {
    ...
    collect([segmentIndex, segmentIndex - 1], function (i, respond) {
      ...
      if (rawSegments.length === 2) {
        ...
        segment.start = prevSegment.duration;
        segment.end = segment.duration;
        segment.duration = segment.duration - prevSegment.duration;
...
// get the info for the most recently create segment of a stream
Streamify.prototype.getLatestSegment = function (streamName, callback) {
  // NOTE: we can get the exact info for any one segment by simply comparing it
  // to its previous segment!
};
