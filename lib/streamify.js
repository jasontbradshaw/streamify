...
// parse a floating point time from a time string and return it in milliseconds
Streamify.parseTimeToMs = function (timeStr) {
  var timeParts = timeStr.split(':');

  // give up if we got invalid information
  ...
  var seconds = parseFloat(timeParts[2], 10);

  // convert all to milliseconds so we don't lose precision, then sum
  var result = Math.round(seconds * 1000);
  ...
  fs.readdir(streamPath, function (err, files) {
    var respond = collect(files.length, function (segments) {
      ...
      _.each(segments, function (segment) {
        ...
        // convert durations to seconds (done last to prevent fp errors)
        segment.start = start / 1000;
        segment.duration = duration / 1000;
        segment.end = end / 1000;
      });
...
Streamify.prototype._getRawSegment = function (streamName, segmentName, callback) {
  ...
  this._getSegmentStats(streamName, segmentName, function (segmentStats) {
    if (segmentStats) {
      ...
      var segment = new models.Segment(self._indexFromSegmentName(segmentName));
      segment.size = segmentStats.size;
      segment.createDate = segmentStats.ctime;
      segment.updateDate = segmentStats.mtime;

      // get detailed info about the segment
      var segmentPath = path.join(self.streamDir, streamName, segmentName);
      var meta = new ffmpeg.Metadata(segmentPath);
      meta.get(function (metadata, err) {
        if (!err) {
          // this duration is off since each segment assumes itself to be a
          // complete file, even though it's missing all preceding segments. we
          // fill in the reported duration here, and calculate the actual
          // duration once all segments are collected.
          segment.duration = Streamify.parseTimeToMs(metadata.durationraw);
