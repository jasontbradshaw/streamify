...
var Streamify = function (config) {
  ...
  this.streamIdRegex = new RegExp('^' + this.streamPrefix);

  this.segmentLengthSeconds = config.segment_length_seconds;
  this.segmentExtension = 'webm';
  this.segmentFileNameRegex = new RegExp('^([0-9]+)\.' + this.segmentExtension + '$');
  ...
  this.io = null;
  this.socket = null;
  this.recordingStreamId = null;
  this.statusCallbackTimeoutMs = 3000;
  this._setupRecorderAndSocket(config);
};

Streamify.prototype._segmentFileNameFromId = function (id) {
  return id + '.' + this.segmentExtension;
};

Streamify.prototype._idFromSegmentFileName = function (fileName) {
  var match = this.segmentFileNameRegex.exec(fileName);
  if (match) {
...
Streamify.prototype._getStreamStats = function (streamId, callback) {
  if (this.streamIdRegex.exec(streamId)) {
    var streamPath = path.join(this.streamDir, streamId);
    fs.stat(streamPath, function (err, streamStats) {
      ...
      if (!err) {
        if (streamStats.isDirectory()) {
          ...
          callback(new Error('invalid stream: ' + streamId));
        }
      } else {
        // otherwise, return an error
        callback(new Error('could not find stream: ' + streamId));
      }
    });
  } else {
    // wasn't a stream
    callback(new Error('invalid stream id: ' + streamId));
  }
};

// returns the fs.stat of a segment if it exists, null otherwise
Streamify.prototype._getSegmentStats = function (streamId, segmentFileName, callback) {
  var self = this;

  if (this.segmentFileNameRegex.exec(segmentFileName)) {
    var segmentPath = path.join(this.streamDir, streamId, segmentFileName);
    fs.stat(segmentPath, function (err, segmentStats) {
      if (!err) {
        if (segmentStats.isFile()) {
          callback(null, segmentStats);
        } else {
          callback(new Error('invalid segment: ' + streamId + '[' +
              self._idFromSegmentFileName(segmentFileName) + ']'));
        }
      } else {
        callback(new Error('could not find segment: ' + streamId + '[' +
            self._idFromSegmentFileName(segmentFileName) + ']'));
      }
    });
  } else {
    callback(new Error('invalid segment file name: ' + segmentFileName));
  }
};
...
Streamify.prototype._getRawSegment = function (streamId, segmentFileName, callback) {
  var self = this;

  this._getSegmentStats(streamId, segmentFileName, function (err, segmentStats) {
    if (!err) {
      // fill in simple segment information
      var rawSegment = new models.Segment(self._idFromSegmentFileName(segmentFileName));
      ...
      var segmentPath = path.join(self.streamDir, streamId, segmentFileName);
      ffprobe(segmentPath, function (err, metadata) {
        if (!err) {
          ...
          callback(new Error('could not get info for segment: ' + streamId +
              '[' + self._idFromSegmentFileName(segmentFileName) + ']'));
        }
      });
    } else {
...
Streamify.prototype._handleStatusHeartbeat = function (recordingStreamId) {
  this.recordingStreamId = recordingStreamId;
};
...
Streamify.prototype.getStream = function (streamId, callback) {
  var self = this;

  this._getStreamStats(streamId, function (err, streamStats) {
    if (!err) {
      // build the stream object so we can fill it out
      var stream = new models.Stream(streamId);

      var streamPath = path.join(self.streamDir, streamId);
      fs.readdir(streamPath, function (err, segments) {
        if (!err) {
          // the number of callbacks that we must wait for before 'returning'
          collect(segments, function (segmentFileName, respond) {
            self._getSegmentStats(streamId, segmentFileName,
...
Streamify.prototype.getSegments = function (streamId, callback) {
  var self = this;
    ...
    if (!err) {
      ...
      fs.readdir(streamPath, function (err, files) {
        if (!err) {
          ...
          collect.chunked(files || [], function (segmentFileName, respond) {
            self._getRawSegment(streamId, segmentFileName,
              function (e, s) { respond(s); });
          }, function (segments) {
            // finish filling out the segments
            segments = _.sortBy(_.compact(segments), 'id');
...
Streamify.prototype.getSegment = function (streamId, segmentId, callback) {
  // if the id is 0, we don't need a preceding segment
  if (segmentId === 0) {
    // get the info from the first segment and return it
    var segmentFileName = this._segmentFileNameFromId(segmentId);
    this._getRawSegment(streamId, segmentFileName, function (err, segment) {
      if (!err) {
    ...
    var self = this;
    collect([segmentId, segmentId - 1], function (i, respond) {
      self._getRawSegment(streamId, self._segmentFileNameFromId(i),
        function (e, s) { respond(s); });
    }, function (rawSegments) {
      rawSegments = _.sortBy(_.compact(rawSegments), 'id');
...
// get a tailing stream of the given segment if it exists, otherwise an error
Streamify.prototype.getSegmentStream = function (streamId, segmentFileName, callback) {
  if (this.segmentFileNameRegex.exec(segmentFileName)) {
    var segmentPath = path.join(this.streamDir, streamId, segmentFileName);
    var self = this;
    fs.exists(segmentPath, function (segmentExists) {
      if (segmentExists) {
        // build and return the tailing readable stream
        callback(null, ts.createReadStream(segmentPath, {
          timeout: 5000 // ms to wait before considering the stream 'done'
        }));
      } else {
        callback(new Error('could not find segment: ' + streamId + '[' +
            self._idFromSegmentFileName(segmentFileName) + ']'));
    ...
    callback(new Error('invalid segment name: ' + segmentFileName));
  }
};
...
Streamify.prototype.record = function (callback) {
  ...
  var statusCallback = function () {
    ...
    if (self.recordingStreamId) {
      // return an up-to-date representation of the stream
      self.getStream(self.recordingStreamId, callback);
    } else {
